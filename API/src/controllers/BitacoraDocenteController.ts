...
export class BitacoraDocenteController {
  static getAll = async (req: Request, res: Response) => {
    try {
      ...
      if (bitacoras.length === 0)
        return res.status(404).json({ message: 'No hay bitacoras activas' });
  ...
  static getById = async (req: Request, res: Response) => {
    try {
      ...
      try {
        ...
        return res
          .status(404)
          .json({ message: 'Bitacora no encontrada o inactiva' });
      }
  ...
  static insert = async (req: Request, res: Response) => {
    try {
      ...
      if (bitacoraExistente)
        return res.status(400).json({ message: 'Bitacora existente' });
      ...
      try {
        ...
        return res
          .status(201)
          .json({ message: 'Bitacora insertada correctamete' });
      } catch (error) {
        return res
          .status(400)
          .json({ message: 'No se pudo insertar la bitacora' });
      }
    } catch (error) {
      return res.status(400).json({ error: error });
    }
  ...
  static update = async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (!bitacoraExistente)
        return res.status(404).json({ message: 'Bitacora inexistente' });
      let bitacora = new BitacoraDocente();
      let fecha = new Date();
      bitacora.Bitacora_Id = Bitacora_Id;
      bitacora.estudiante = Estudiante_Id;
      ...
      try {
        ...
        return res
          .status(200)
          .json({ message: 'Bitacora actualizada correctamete' });
      } catch (error) {
        return res
          .status(400)
          .json({ message: 'No se pudo actualizar la bitacora' });
      }
    } catch (error) {
      return res.status(400).json({ error: error });
    }
  ...
  static delete = async (req: Request, res: Response) => {
    try {
      ...
      try {
        ...
        return res
          .status(200)
          .json({ message: 'Bitacora eliminada correctamente' });
      } catch (error) {
        return res
          .status(400)
          .json({ message: 'No se pudo eliminar la bitacora' });
      }
    } catch (error) {
      return res.status(400).json({ error: error });
    }
