  ...
  static tipoDeProblema = async (req: Request, res: Response) => {
    try {
      ...
      if (Estudiantes.length === 0)
        return res.status(404).json({ msg: 'No hay estudiantes' });
      const Citas = await repoCitas.find({
        select: ['Cita_Id', 'Encargado_Nombre', 'Fecha_Cita'],
      });
      if (Citas.length === 0)
        return res.status(404).json({ msg: 'No hay citas' });
      ...
      if (Seguimientos.length === 0)
        return res.status(404).json({ msg: 'No hay seguimientos' });
      ...
      const evaluacionesEstudiante = Evaluaciones.map((evaluacion) => ({
        ...
        Estudiante: {
          Estudiante_Id: evaluacion.estudiante
      ...
      const bitacorasEstudiante = Bitacoras.map((bitacora) => ({
        ...
        Estudiante: {
          Estudiante_Id: bitacora.estudiante
            ? bitacora.estudiante.Estudiante_Id
            : null,
        },
      }));
      if (Bitacoras.length === 0)
        return res.status(404).json({ msg: 'No hay bitacoras' });
      const reporte = {
        Estudiantes,
        Citas,
    ...
    try {
      ...
      const estudiantesConEdad = Estudiantes.map((estudiante) => {
        const edad = differenceInYears(
          new Date(),
          parseISO(estudiante.Fecha_Nacimiento.toISOString())
        );
        return {
          ...estudiante,
          Edad: edad,
