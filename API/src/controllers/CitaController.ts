...
export class CitaController {
    ...
    static getAll = async (req: Request, res: Response) => {
        try {
            ...
            const citas = await citasRepo.find({
                where: { Estado: true }, relations: { estudiante: true }
            });
            if (citas.length === 0) return res.status(404).json({ message: 'No hay citas activas' });
    ...
    static getById = async (req: Request, res: Response) => {
        try {
            ...
            try {
                ...
                return res.status(404).json({ message: 'Cita no encontrada o inactiva' });
            }
    ...
    static insert = async (req: Request, res: Response) => {
        ...
        */
        try {
            const { Cita_Id, Estudiante_Id, Encargado_Nombre,
                Aprobacion_Cita, Fecha_Cita } = req.body;
            const citasRepo = AppDataSource.getRepository(Cita);
            const citaExistente = await citasRepo.findOne({ where: { Cita_Id, Estado: true } });
            if (citaExistente) return res.status(400).json({ message: 'Cita existente' });
            let cita = new Cita();
        ...
        */
        try {
            ...
            const citaExistente = await citasRepo.findOne({
                where: { Cita_Id }, relations: { estudiante: true }
            });
            if (!citaExistente) return res.status(404).json({ message: 'Cita inexistente' });
            let cita = new Cita();
            cita.Cita_Id = Cita_Id;
            cita.estudiante = Estudiante_Id;
            cita.Encargado_Nombre = Encargado_Nombre;
    ...
    static delete = async (req: Request, res: Response) => {
        try {
            const Cita_Id = parseInt(req.params['id']);
            if (!Cita_Id) return res.status(400).json({ message: 'Debe indicar el id' });
            const citasRepo = AppDataSource.getRepository(Cita);
            let cita: Cita;
            try {
                cita = await citasRepo.findOneOrFail({ where: { Cita_Id, Estado: true } });
            } catch (error) {
